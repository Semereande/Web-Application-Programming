...
async function populateDropdown(selectId) {
  ...
  async function deleteStudent() {
    const selectedStudentId = document.getElementById("ddlStudent").value;
    ...
    const response = await fetch(`http://localhost:3012/students/${selectedStudentId}`, deleteRequest);
  
    if (response.ok) {
      // Remove the row from the table
      const row = document.getElementById(selectedStudentId);
      if (row) {
        row.remove();
      } else {
  ...
  async function updateStudent() {
    ...
    const response = await fetch(`http://localhost:3012/students/${selectedStudentId}`, updateRequest);
  
    if (response.ok) {
      const row = document.getElementById(selectedStudentId);
      // Update the cells with the new data
      const cells = row.getElementsByTagName("td");
      cells[0].textContent = updatedId;
      cells[1].textContent = updatedName;
      cells[2].textContent = updatedProgram;
...
document.getElementById("btnDelete").addEventListener("click", () => {
    const ddlStudent = document.getElementById("ddlStudent");
    const selectedId = ddlStudent.value;
